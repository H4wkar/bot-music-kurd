...
btnn =[
    [Button.url("پشتگیری کردن", url=f"t.me/{Config.SUPPORT}"), Button.url("که‌ناڵ", url=f"t.me/{Config.CHANNEL}")],
...
async def play(event):
    ...
    if (
      ...
      "**- ناونیشانی داواکراو نەدۆزرایەوە ناونیشانێکی تر بە دروستی بنووسە**"
        ...
        botman = await event.edit("📥 **جارێ دابەزاندن**")
        ...
        if chat_id in QUEUE:
            ...
            try:
                ...
                caption = f"🏷 ** ناونيشان** [{songname}]({link})\n**ئایدی گرووپ**: `{chat_id}`\n💡 **چالاکه:** بێ کێشه یه\n🎧 **فەرمان بە**: {from_user}"
...
#end
@JE313P.on(events.NewMessage(pattern="^[/?!]اوقف"))
@is_admin
...
async def vplay(event):
    ...
    if (
        ...
        return await event.client.send_file(chat_id, Config.CMD_IMG, caption="**پێویسته به ناونیشانی راست بێنوسی **\n\n **مثال**: `!فيديو قران`", buttons=btnn)
...
async def vc_playlist(event, perm):
    ...
    if chat_id in QUEUE:
        ...
        await event.reply("**تکایە کەمێک چاوەڕێ بکە**")
...
#كود المغادرة
@JE313P.on(events.NewMessage(pattern="^[?!/]cstop"))
@is_admin
...
async def vc_skip(event, perm):
    ...
    if len(event.text.split()) < 2:
        ...
        if op == 0:
            ...
            await event.reply("کۆتایی هاتووە بۆیە پەیوەندیەکەم بەجێهێشت, 10")
        else:
            await event.reply(
